<?php
	...
	echo "Please provide the company name for the site: ";
	$handle = fopen("php://stdin","r");
	$company_name = fgets($handle);
	$_REQUEST['company_name'] = trim ($company_name);

	echo "Please provide the domain name for the site: ";
	$handle = fopen("php://stdin","r");
	$domain = fgets($handle);
	$_SERVER['HTTP_HOST'] = trim($domain);

	echo "Please provide the admin username for the site: ";
	$handle = fopen("php://stdin","r");
	$admin_login = fgets($handle);
	$_REQUEST['admin_login'] = trim ($admin_login);

	echo "Please provide the admin password for the site: ";
	$handle = fopen("php://stdin","r");
	$password_1 = fgets($handle);
	$_REQUEST['password_1'] = trim($password_1);
	$_REQUEST['password_2'] = trim($password_1);

		...
		<form name="register" id="register_form" action="<?= PATH ?>/_register/account" method="POST">
			...
			<?php if (!$readOnly) { ?>
				...
				<section class="form-group form-actions">
					<input type="submit" name="method" value="Apply" class="button submitButton registerSubmitButton" onclick="return submitForm();" <?php if ($readOnly) echo 'disabled'; ?> />
					<input type="button" name="method" value="Change Password" class="button submitButton registerSubmitButton" onclick="return passChange();" <?php if ($readOnly) echo 'disabled'; ?> />
				</section>
			<?php } ?>

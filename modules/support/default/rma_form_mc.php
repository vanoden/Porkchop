...
$organization = $rma->item ()->request->customer->organization;
...
if ($page->errorCount() < 1 && $_REQUEST ['form_submitted'] == 'submit') {
	...
	if (! $shippingShipment->id) {
		...
		$parameters ['code'] = $rmaCode;
		$parameters ['document_number'] = sprintf("RMA%06d",$rmaCode);
		...
		if (! $registerLocationShipping->id) {
			...
			if (! $shippingShipment->add ( $parameters )) {
				...
				if ($shippingShipment->error()) {
					...
					if ($shippingShipment->error()) $page->addError("Error adding item to shipment: ".$shippingShipment->error());
